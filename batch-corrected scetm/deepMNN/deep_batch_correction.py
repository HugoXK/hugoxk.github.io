...
import numpy as np
import scanpy as sc
from .model import Net, my_dataset, AutoEncoder,my_train
...
def training_step(net, optim, batch, PCs,my_module, beta1=BETA1, beta2=BETA2):
    ...
    latent_var1 = my_module(corrected_batch1)
    latent_var2 = my_module(corrected_batch2)
    ...
    loss1 = torch.sum(torch.mean(torch.abs(latent_var1[0] - latent_var2[0]), axis=1))
    ...
    loss = BETA0 * loss1 + 0 * loss2 + beta2 * loss3
...
def correct_scanpy(adatas,
    ...
    torch_data = torch.from_numpy(adatas[0].X)
    
    torch_data = torch_data.to(device)
    my_datasets = my_dataset(torch_data)
    my_module = AutoEncoder().to(device)
    my_dataloader = Data.DataLoader(dataset= my_datasets, batch_size= BATCH_SIZE, shuffle= False)

    my_train(my_module,my_dataloader)
    
    
    my_tensor,_ = my_module(torch_data) 
    my_tensor = my_tensor.cpu().detach().numpy()
    my_tensor = np.array(my_tensor)
    
    my_module.requires_grad_(False)
    ...
    # alignments, matches = find_alignments(my_tensor, knn=KNN, approx=APPROX, verbose=VERBOSE, alpha=ALPHA)
    ...
    for epoch in range(max_epochs):
        ...
        for _, batch in enumerate(data_loader):
            ...
            batch_loss = training_step(net, optim, batch_data, PCs_tensor,my_module=my_module)
